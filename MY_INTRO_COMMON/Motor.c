...
static void MOT_PrintStatus(const CLS1_StdIOType *io) {
	...
	UTIL1_strcat(buf, sizeof(buf), (unsigned char*) (
			MOT_GetDirection(&motorL) == MOT_DIR_FORWARD ? ", fw" : ", bw"));
	...
	UTIL1_strcat(buf, sizeof(buf), (unsigned char*) (
			MOT_GetDirection(&motorR) == MOT_DIR_FORWARD ? ", fw" : ", bw"));
	CLS1_SendStr(buf, io->stdOut);
	CLS1_SendStr((unsigned char*) "\r\n", io->stdOut);
}
...
uint8_t MOT_ParseCommand(const unsigned char *cmd, bool *handled, const CLS1_StdIOType *io) {
	...
	if (UTIL1_strcmp((char*)cmd, (char*)CLS1_CMD_HELP) == 0
			|| UTIL1_strcmp((char*)cmd, (char*)"motor help") == 0) {
		MOT_PrintHelp(io);
		*handled = TRUE;
	} else if (UTIL1_strcmp((char*)cmd, (char*)CLS1_CMD_STATUS) == 0
			|| UTIL1_strcmp((char*)cmd, (char*)"motor status") == 0) {
		MOT_PrintStatus(io);
		*handled = TRUE;
	...
	//---------------------------------
	else if (UTIL1_strncmp((char*)cmd, (char*)"motor RL duty ", sizeof("motor RL duty ")-1) == 0) {
		p = cmd + sizeof("motor RL duty");
		if (UTIL1_xatoi(&p, &val) == ERR_OK && val >= -100 && val <= 100) {
			MOTS_SetSpeedPercent(val);
			*handled = TRUE;
		} else {
			CLS1_SendStr((unsigned char*) "Wrong argument, must be in the range -100..100\r\n", io->stdErr);
			res = ERR_FAILED;
		}
	} else if (UTIL1_strcmp((char*)cmd, (char*)"motor RL forward") == 0) {
...
#endif /* PL_HAS_SHELL */
//-------------------------------------------------------------------------------------
void MOTS_SetSpeedPercent(MOT_SpeedPercent percent) {
	MOT_SetSpeedPercent(&motorL, (MOT_SpeedPercent) percent);
	MOT_SetSpeedPercent(&motorR, (MOT_SpeedPercent) percent);
}
void MOTS_SetDirection(MOT_Direction dir) {
	MOT_SetDirection(&motorL, (MOT_Direction) dir);
	MOT_SetDirection(&motorR, (MOT_Direction) dir);
}
void MOTS_Turn(MOT_Turn_Direction dir,int speed) {
	if (dir == MOT_TURN_RIGHT)

	{
		MOTS_SetSpeedPercent(speed);
		MOT_SetDirection(&motorL, MOT_DIR_FORWARD);
		MOT_SetDirection(&motorR, MOT_DIR_BACKWARD);
	} else {
		MOTS_SetSpeedPercent(speed);
